...
export default class PropietariosRepositoryPostgres implements PropietariosRepository{
    ...
    async addPropietario(propietario: Propietario): Promise<Propietario> {
        const id: any[] = await executeQuery(`insert into propietarios (nombre,apellidos,cif,correo,telefono,poblacion) values ('${propietario.nombre}','${propietario.apellidos}','${propietario.cif}','${propietario.correo}',${propietario.telefono},'${propietario.poblacion}') returning id`)
        try{
    ...
    async modifyPropietario(propietario: Propietario, id: Number): Promise<Propietario> {
        try{
            await executeQuery(`update propietarios set nombre = '${propietario.nombre}',apellidos = '${propietario.apellidos}',cif='${propietario.cif}',correo='${propietario.correo}',telefono=${propietario.telefono},poblacion ='${propietario.poblacion}' where id = ${id}`)
            return{
                id: id,
